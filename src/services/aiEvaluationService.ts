...
const buildSystemPrompt = (rubric: Rubric): string => {
  const criteriaList = rubric.criteria
    .map(c => `- ${c.name} (ID: ${c.id}, Max Score: ${c.levels.find(l => l.id === 'excellent')?.score || 5})`)
    .join('\n');

  const totalScore = rubric.criteria.reduce((sum, c) => sum + (c.levels.find(l => l.id === 'excellent')?.score || 5), 0);
...
- \`rubric_breakdown\`: كائن (object) حيث كل مفتاح هو \`id\` أحد المعايير (مثل '${rubric.criteria[0].id}', '${rubric.criteria[1].id}', إلخ). يجب أن تكون قيمة كل مفتاح كائنًا آخر يحتوي على:
